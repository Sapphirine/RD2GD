...
import datamodel.DescriptionTextArea;
import datamodel.Field;
import datamodel.Table;
import datamodel.TableCellLongTextRenderer;
...
public class DetailsPanel extends JPanel implements DetailsListener
{
	...
	private static final long serialVersionUID = -5035165879701943422L;
	...
	private class TablePanel extends JPanel implements DocumentListener 
	{
		...
		public TablePanel() 
		{
			...
			displayTable.getColumnModel().addColumnModelListener
			(
		...
		public void showTable(Table table) 
		{
			...
			{
				fieldTable.add(field.getName(), field.getFullType(),field.getDescription());
			}
	...
	private class FieldPanel extends JPanel implements DocumentListener 
	{
		...
		private SimpleTableModel	valueTable			= new SimpleTableModel("Value", "Frequency", "Relative Frequency (%)");
		//private JTextArea			commentsArea		= new JTextArea();
		private Field				field;
		...
		public FieldPanel() 
		{
			...
			fieldListPanel.setBorder(BorderFactory.createTitledBorder("Values"));
			add(fieldListPanel, BorderLayout.CENTER);
		...
		public void showField(Field field) 
		{
			...
			rowCountLabel.setText(field.getFullType());
			...
			DecimalFormat formatter = new DecimalFormat("#,###");
			DecimalFormat formatterPercent = new DecimalFormat("#,##0.00");
			
			for(Value value : field.getValues()) 
			{
				valueTable.add(value.getName(), formatter.format(value.getFrequency()), 
						formatterPercent.format(value.getRelativeFrequency()));
			}
	...
	private class ItemToItemMapPanel extends JPanel implements DocumentListener {
		...
		public void showItemToItemMap(ItemToItemMap itemToItemMap) {
			...
			sourceLabel.setText(itemToItemMap.getRelationalItem().toString());
			targetLabel.setText(itemToItemMap.getGraphItem().toString());
	...
	{
		private static final long 			serialVersionUID = 3974457730117699895L;
